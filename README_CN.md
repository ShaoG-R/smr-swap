...
|------|----------|---------|---------|
| 单线程读取 | 0.90 ns | 9.20 ns | **快 90%** |
| 单线程写入 | 66.61 ns | 104.97 ns | **快 37%** |
| 多线程读取 (2 线程) | 0.90 ns | 9.24 ns | **快 90%** |
| 多线程读取 (4 线程) | 0.94 ns | 9.47 ns | **快 90%** |
| 多线程读取 (8 线程) | 0.93 ns | 9.75 ns | **快 90%** |
| 混合读写 (1写+2读) | 56.36 ns | 426.24 ns | **快 87%** |
| 混合读写 (1写+4读) | 56.52 ns | 428.05 ns | **快 87%** |
| 混合读写 (1写+8读) | 58.21 ns | 509.31 ns | **快 89%** |
| 批量读取 | 1.63 ns | 10.19 ns | **快 84%** |
| 持有守卫时读取 | 54.57 ns | 891.63 ns | **快 94%** |
| 内存压力下读取 | 873.52 ns | 1.63 µs | **快 46%** |

### 单写者不同读写比例场景（1 Writer + 2 Readers）

| 读写比例 | SMR-Swap | Mutex | 说明 |
|----------|----------|-------|------|
| 100:1 | 171.54 ns | 6.15 µs | SMR 比 Mutex 快 97% |
| 10:1 | 67.64 ns | 739.97 ns | SMR 比 Mutex 快 91% |
| 1:1 | 55.96 ns | 145.27 ns | SMR 比 Mutex 快 61% |
| 1:10 | 554.74 ns | 231.97 ns | Mutex 比 SMR 快 58% |
| 1:100 | 5.90 µs | 1.13 µs | Mutex 比 SMR 快 81% |

### 多写多读场景（SMR-Swap 使用 Mutex 包装）

| 配置 | SMR-Swap | Mutex | ArcSwap | 说明 |
|------|----------|-------|---------|------|
| 4写+4读 | 559.87 ns | 503.32 ns | 1.85 µs | SMR 比 ArcSwap 快 70% |
| 4写+8读 | 564.80 ns | 825.97 ns | 2.17 µs | SMR 比 ArcSwap 快 74% |
| 4写+16读 | 563.60 ns | 1.20 µs | 2.81 µs | SMR 比 ArcSwap 快 80% |

### 操作开销

| 操作 | SMR-Swap | ArcSwap | Mutex | 说明 |
|------|----------|---------|-------|------|
| 创建 (Creation) | ~214 ns | ~130 ns | ~50 ns | SMR 初始化成本较高 |
| 销毁 (Drop) | ~65 ns | ~110 ns | ~42 ns | SMR 销毁比 ArcSwap 快 |
| 句柄克隆 (Handle Clone) | ~56 ns | ~9 ns | ~9 ns | LocalReader 克隆比 Arc 克隆重 |
| 本地状态检查 | ~0.2 ns | N/A | N/A | `is_pinned`/`version` 检查极快 |

### 性能分析

- **读取性能卓越**：单线程和多线程读取都保持亚纳秒级延迟（~0.9 ns），比 ArcSwap 快约 10 倍
- **线性扩展**：多线程读取性能几乎不随线程数增加而下降
- **混合负载稳定**：在 1 写 + N 读场景下，SMR-Swap 保持稳定的 ~57 ns 延迟
- **读多写少场景最优**：读写比 100:1 和 10:1 时，SMR-Swap 全面领先，比 Mutex 快 91-97%
- **写多读少场景**：写入占比较高时（1:10、1:100），Mutex 写入效率更高
- **多写场景卓越**：即使使用 Mutex 包装，SMR-Swap 在多写多读场景下仍比 ArcSwap 快 70-80%
- **内存压力表现良好**：激进的 GC 策略确保内存压力下性能稳定
- **操作开销**：创建/克隆成本略高（对比 Arc），由于不需要引用计数原子操作，销毁速度更快；本地状态检查几乎无开销
