...
|------|----------|---------|---------|
| 单线程读取 | 0.91 ns | 9.78 ns | **快 90%** |
| 单线程写入 | 89.51 ns | 127.47 ns | **快 30%** |
| 多线程读取 (2 线程) | 0.91 ns | 9.14 ns | **快 90%** |
| 多线程读取 (4 线程) | 0.92 ns | 9.64 ns | **快 90%** |
| 多线程读取 (8 线程) | 0.96 ns | 9.85 ns | **快 90%** |
| 混合读写 (1写+2读) | 87.63 ns | 457.74 ns | **快 81%** |
| 混合读写 (1写+4读) | 87.58 ns | 458.89 ns | **快 81%** |
| 混合读写 (1写+8读) | 88.07 ns | 497.94 ns | **快 82%** |
| 批量读取 | 1.65 ns | 9.56 ns | **快 83%** |
| 持有守卫时读取 | 84.69 ns | 931.91 ns | **快 91%** |
| 内存压力下读取 | 914.20 ns | 1.69 µs | **快 46%** |

### 单写者不同读写比例场景（1 Writer + 2 Readers）

| 读写比例 | SMR-Swap | ArcSwap | Mutex | 说明 |
|----------|----------|---------|-------|------|
| 100:1 | 168.00 ns | 1.18 µs | 6.96 µs | SMR 比 ArcSwap 快 86%，比 Mutex 快 98% |
| 10:1 | 93.70 ns | 333.72 ns | 951.22 ns | SMR 比 ArcSwap 快 72%，比 Mutex 快 90% |
| 1:1 | 87.41 ns | 255.29 ns | 155.28 ns | SMR 比 ArcSwap 快 66%，比 Mutex 快 44% |
| 1:10 | 849.17 ns | 2.87 µs | 620.80 ns | SMR 比 ArcSwap 快 70%，Mutex 比 SMR 快 27% |
| 1:100 | 8.79 µs | 28.61 µs | 5.26 µs | SMR 比 ArcSwap 快 69%，Mutex 比 SMR 快 40% |

### 多写多读场景（SMR-Swap 使用 Mutex 包装）

| 配置 | SMR-Swap | Mutex | ArcSwap | 说明 |
|------|----------|-------|---------|------|
| 4写+4读 | 542.29 ns | 1.13 µs | 1.96 µs | SMR 比 ArcSwap 快 72% |
| 4写+8读 | 566.80 ns | 1.44 µs | 2.18 µs | SMR 比 ArcSwap 快 74% |
| 4写+16读 | 548.84 ns | 1.87 µs | 2.89 µs | SMR 比 ArcSwap 快 81% |

### 性能分析

- **读取性能卓越**：单线程和多线程读取都保持亚纳秒级延迟（~0.9 ns），比 ArcSwap 快约 10 倍
- **线性扩展**：多线程读取性能几乎不随线程数增加而下降
- **混合负载稳定**：在 1 写 + N 读场景下，SMR-Swap 保持稳定的 ~88 ns 延迟
- **读多写少场景最优**：读写比 100:1 和 10:1 时，SMR-Swap 全面领先，比 ArcSwap 快 72-86%，比 Mutex 快 90-98%
- **写多读少场景**：写入占比较高时（1:10、1:100），Mutex 写入效率更高，但 SMR-Swap 仍比 ArcSwap 快 69-70%
- **多写场景卓越**：即使使用 Mutex 包装，SMR-Swap 在多写多读场景下仍比 ArcSwap 快 72-81%
