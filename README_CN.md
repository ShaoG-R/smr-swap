...
|------|----------|---------|---------|
| 单线程读取 | 0.90 ns | 9.19 ns | **快 90%** |
| 单线程写入 | 89.81 ns | 104.79 ns | **快 14%** |
| 多线程读取 (2 线程) | 0.90 ns | 9.19 ns | **快 90%** |
| 多线程读取 (4 线程) | 0.92 ns | 9.30 ns | **快 90%** |
| 多线程读取 (8 线程) | 0.94 ns | 9.60 ns | **快 90%** |
| 混合读写 (1写+2读) | 86.01 ns | 424.02 ns | **快 80%** |
| 混合读写 (1写+4读) | 86.03 ns | 426.10 ns | **快 80%** |
| 混合读写 (1写+8读) | 86.75 ns | 502.69 ns | **快 83%** |
| 批量读取 | 1.62 ns | 9.58 ns | **快 83%** |
| 持有守卫时读取 | 84.96 ns | 886.14 ns | **快 90%** |
| 内存压力下读取 | 781.29 ns | 1.62 µs | **快 52%** |

### 单写者不同读写比例场景（1 Writer + 2 Readers）

| 读写比例 | SMR-Swap | RwLock | Mutex | 说明 |
|----------|----------|--------|-------|------|
| 100:1 | 251.49 ns | 5.79 µs | 6.18 µs | SMR 比 Mutex 快 96% |
| 10:1 | 101.09 ns | 654.10 ns | 715.95 ns | SMR 比 Mutex 快 86% |
| 1:1 | 87.26 ns | 129.99 ns | 147.57 ns | SMR 比 Mutex 快 41% |
| 1:10 | 857.83 ns | 233.59 ns | 230.25 ns | Mutex 比 SMR 快 73% |
| 1:100 | 8.52 µs | 1.04 µs | 999.18 ns | Mutex 比 SMR 快 88% |

### 多写多读场景（SMR-Swap 使用 Mutex 包装）

| 配置 | SMR-Swap | Mutex | ArcSwap | 说明 |
|------|----------|-------|---------|------|
| 4写+4读 | 2.03 µs | 459.86 ns | 1.82 µs | Mutex 最快 |
| 4写+8读 | 2.10 µs | 778.83 ns | 2.22 µs | Mutex 最快 |
| 4写+16读 | 2.04 µs | 1.25 µs | 2.83 µs | SMR 比 ArcSwap 快 28% |

### 操作开销

| 操作 | SMR-Swap | ArcSwap | Mutex | 说明 |
|------|----------|---------|-------|------|
| 创建 (Creation) | ~159 ns | ~136 ns | ~49 ns | SMR 初始化成本较高 |
| 销毁 (Drop) | ~78 ns | ~108 ns | ~41 ns | SMR 销毁速度快于 ArcSwap |
| 句柄克隆 (Handle Clone) | ~57 ns | ~9 ns | ~9 ns | LocalReader 克隆比 Arc 克隆重 |
| 本地状态检查 | ~0.18 ns | N/A | N/A | `is_pinned`/`version` 检查极快 |

### 性能分析

- **读取性能卓越**：单线程和多线程读取都保持亚纳秒级延迟（~0.9 ns），比 ArcSwap 快约 10 倍
- **线性扩展**：多线程读取性能几乎不随线程数增加而下降
- **混合负载稳定**：在 1 写 + N 读场景下，SMR-Swap 保持 ~86 ns 延迟，显著优于 ArcSwap (~425ns)
- **读多写少场景最优**：读写比 100:1 和 10:1 时，SMR-Swap 全面领先，比 Mutex 快 86-96%
- **写多读少场景**：写入占比较高时（1:10、1:100），Mutex 写入效率更高
- **多写场景表现**：在多写多读场景下，Mutex 通常性能最佳，但在高并发读取（16+读）下，SMR-Swap 扩展性优于 ArcSwap
- **内存压力表现良好**：激进的 GC 策略确保内存压力下性能稳定
- **操作开销**：创建/克隆成本略高（对比 Arc），但销毁速度更快；本地状态检查几乎无开销
