...
fn bench_single_thread_write(c: &mut Criterion) {
    ...
    // SMR-Swap 单线程写入
    group.bench_function("smr_swap", |b| {
        let (mut swapper, _reader) = smr_swap::new(vec![1; 1000]);
        let mut counter = 0;
        b.iter(|| {
            counter += 1;
            swapper.update(vec![counter; 1000]);
        });
    });
...
fn bench_mixed_read_write(c: &mut Criterion) {
    ...
    for num_readers in [2, 4, 8].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &num_readers| {
                b.iter_custom(|iters| {
                    let (mut swapper, reader) = smr_swap::new(vec![1; 1000]);
                    let reader = Arc::new(reader);
...
fn bench_read_under_memory_pressure(c: &mut Criterion) {
    ...
    // SMR-Swap: 频繁写入导致内存压力
    group.bench_function("smr_swap", |b| {
        let (mut swapper, reader) = smr_swap::new(vec![0; 10000]);
        let reader = Arc::new(reader);
