...
fn bench_single_thread_read(c: &mut Criterion) {
    ...
    group.bench_function("smr_swap", |b| {
        let (_swapper, reader) = smr_swap::new(Arc::new(vec![1; 1000]));
        b.iter(|| {
...
fn bench_single_thread_write(c: &mut Criterion) {
    ...
    // SMR-Swap 单线程写入
    group.bench_function("smr_swap", |b| {
        let (mut swapper, _reader) = smr_swap::new(Arc::new(vec![1; 1000]));
        let mut counter = 0;
        b.iter(|| {
            counter += 1;
            swapper.update(Arc::new(vec![counter; 1000]));
        });
    });
...
fn bench_mixed_read_write(c: &mut Criterion) {
    ...
    for num_readers in [2, 4, 8].iter() {
        ...
        group.bench_with_input(
            ...
            |b, &num_readers| {
                b.iter_custom(|iters| {
                    let (mut swapper, reader) = smr_swap::new(Arc::new(vec![1; 1000]));
                    let reader = Arc::new(reader);
                    ...
                    thread::scope(|s| {
                        ...
                        s.spawn(|| {
                            for i in 0..iters {
                                swapper.update(Arc::new(vec![i as u32; 1000]));
                            }
                        });
...
fn bench_read_under_memory_pressure(c: &mut Criterion) {
    ...
    // SMR-Swap: 频繁写入导致内存压力
    group.bench_function("smr_swap", |b| {
        let (mut swapper, reader) = smr_swap::new(Arc::new(vec![0; 10000]));
        let reader = Arc::new(reader);

        // 预先进行大量写入以积累垃圾
        for i in 0..1000 {
            swapper.update(Arc::new(vec![i; 10000]));
        }
        ...
        b.iter_custom(|iters| {
            ...
            thread::scope(|s| {
                ...
                // 同时进行写入
                for i in 0..iters {
                    swapper.update(Arc::new(vec![i as u32; 10000]));
                }
            });
            start.elapsed()
        });
